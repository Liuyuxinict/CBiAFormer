...
class food101(Dataset):
    def __init__(self, root_path, transform=None, train=True, use_name = False, use_multi = True):

        self.root_path = root_path
        if train:
            self.ingredient_path = os.path.join(root_path, "train_ingredient.txt")
            self.class_path = os.path.join(root_path, "train_full.txt")
        else:
            self.ingredient_path = os.path.join(root_path, "test_ingredient.txt")
            self.class_path = os.path.join(root_path, "test_full.txt")

        fp = open(self.ingredient_path, "r")
        multi_label_list = fp.readlines()

        fs = open(self.class_path,"r")
        label_list = fs.readlines()

        img = []
        label = []
        multi_label = []
        for i in range(len(multi_label_list)):
            line = multi_label_list[i]
            line = line.rstrip()
            line = line.split(" ")
            img.append(line[0])
            
            if use_multi:
                multi_label.append([int(i) for i in line[1:]])

            line2 = label_list[i]
            line2 = line2.rstrip()
            line2 = line2.split(" ")
            label.append(int(line2[1]))

        self.paths = img
        self.labels = np.array(label)
        self.multi_label = np.array(multi_label)
        self.train = train
        self.transform = transform
    ...
    def __getitem__(self, item):
        img_path = os.path.join(self.root_path, "images", self.paths[item])
        ...
        if self.use_name:
            ...
            return img, label, self.multi_label[item]
        else:
            return img, label
...
class food172(Dataset):
    def __init__(self, root, transform=None, train=True, use_multi = True):  ##
        self.root = root
        self.img_list = []
        self.transform = transform
        self.phase = 'train' if train == True else 'val'  ##
        self.get_anno()
        self.num_classes = len(self.cat2idx)
        self.use_multi_label = use_multi

    def get_anno(self):
        list_path = os.path.join(self.root, 'retrieval_dict', '{}_anno.json'.format(self.phase))
        self.img_list = json.load(open(list_path, 'r'))
        self.cat2idx = json.load(open(os.path.join(self.root, 'retrieval_dict', 'category.json'), 'r'))
    ...
    def get(self, item):
        ...
        label = int(filename.rstrip().split("/")[0]) - 1

        img = Image.open(os.path.join(self.root, 'images', filename)).convert('RGB')
        if self.transform is not None:
            img = self.transform(img)
        labels = sorted(item['labels'])
        multi_label = np.zeros(self.num_classes, dtype=np.int)
        multi_label[labels] = 1
        # pdb.set_trace()

        return img, label, multi_label

class food200(Dataset):
    def __init__(self, root, transform=None, train=True, use_multi = True):  ##
        self.root = root
        self.img_list = []
        self.transform = transform
        ...
        class_idx = os.path.join(self.root, 'retrieval_dict', 'class_list.txt')
        files = open(class_idx, "r")
        classlist = files.readlines()

        self.class_to_idx = {}
        for item in classlist:
            classname, idx = item.split("\t")
            self.class_to_idx[classname] = int(idx.rstrip())
    ...
    def get(self, item):
        filename = item['file_name']
        label = self.class_to_idx[filename.rstrip().split("/")[0]]
