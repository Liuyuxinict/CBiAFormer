...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        #(batch, (224/4)*(224/4), 96)   共有(224/4)*(224/4)个patch
        ...
        for layer in self.layers:
            x = layer(x)
        # (batch, 49（7*7）, 768)  768:隐藏层维度  49：最后一层的特征数目
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            part_loss = loss_fct(x, labels.view(-1))
            loss = part_loss
            return loss, x
        else:
            return x
...
def swin_small_patch4_window7_224(pretrained = "", **kwargs):
    model_kwargs = dict(
