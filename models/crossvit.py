...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, multi_conv=False):
        ...
        if multi_conv:
            if patch_size[0] == 12:    #small branch
            ...
            elif patch_size[0] == 16:   #large branch
                self.proj = nn.Sequential(
                    nn.Conv2d(in_chans, embed_dim // 4, kernel_size=7, stride=4, padding=3),
                    nn.ReLU(inplace=True),
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            part_loss = loss_fct(ce_logits.view(-1, self.num_classes), labels.view(-1))
            loss = part_loss
            return loss, ce_logits
        else:
            return ce_logits
...
@register_model
def crossvit_18_dagger_384(pretrained="", **kwargs):
    model = VisionTransformer(img_size=[408, 384],
                              patch_size=[12, 16], in_chans=3, embed_dim=[224, 448], depth=[[1, 6, 0], [1, 6, 0], [1, 6, 0]],
                              num_heads=[7, 7], mlp_ratio=[3, 3, 1], qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), multi_conv=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained is not None:
        ...
        for k, v in model_dict.items():
            # if k in dict1.keys():
            if k in pretrained.keys() and "head" not in k:
                state_dict[k] = pretrained[k]
            else:
                state_dict[k] = v
                #print(k)
        model.load_state_dict(state_dict)
