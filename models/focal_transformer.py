...
def focal_tiny_224_window7(pretrained=None, **kwargs):
    model_kwargs = dict(
         window_size=7, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
        focal_levels=[2, 2, 2, 2], focal_windows=[7, 5, 3, 1], expand_sizes=[3, 3, 3, 3], **kwargs)
    model = FocalTransformer(**model_kwargs)
...
if __name__ == '__main__':
    ...
    model = focal_tiny_224_window7(pretrained="E:/Pretrained_model/focal-tiny-is224-ws7.pth",
                            num_classes=101)
    print(model(x).shape)
    ...
    model = FocalTransformer(img_size=img_size, embed_dim=128, depths=[2,2,18,2], drop_path_rate=0.2, 
        ...
        use_conv_embed=True, use_shift=False,
    ).cuda()
    state = torch.load("E:/Pretrained_model/focal_food101_ckpt_epoch_95.pth", map_location='cpu')
    filtered_dict = {k: v for k, v in state['model'].items() if k != "head.weight" and k != "head.bias"}
    model.load_state_dict(filtered_dict, strict=False)
