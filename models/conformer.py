...
class Conformer(nn.Module):
    ...
    def forward(self, x, labels = None):
        ...
        if labels is not None:
            ...
            conv_loss = loss_fct(conv_cls, labels.view(-1))
            tran_loss = loss_fct(tran_cls, labels.view(-1))
            loss = conv_loss + tran_loss
            return loss, tran_cls + conv_cls

        else:
            return tran_cls + conv_cls

@register_model
def Conformer_base_patch16(pretrained="E:/Pretrained_model/Conformer_base_patch16.pth", **kwargs):
