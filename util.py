...
class AveragePrecisionMeter(object):
    ...
    def reduce_all(self, nprocs):
        #rs = self.scores
        #rt = self.targets
        #print(self.scores)
        #print(self.scores.shape)
        scores_gather_list = [torch.zeros_like(self.scores) for _ in range(nprocs)]
        target_gather_list = [torch.zeros_like(self.targets) for _ in range(nprocs)]

        torch.distributed.all_gather(scores_gather_list, self.scores)
        torch.distributed.all_gather(target_gather_list, self.targets)

        rs = torch.cat(scores_gather_list, dim=0)
        rt = torch.cat(target_gather_list, dim=0)
    ...
    def value(self):
        ...
        for k in tqdm(range(self.scores.size(1))):
            #print(k)
            # sort scores
